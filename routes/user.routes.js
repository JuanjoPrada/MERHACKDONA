...
router.get('/edit-profile', isLoggedIn, checkRoles('CLIENT'), (req, res, next) => {
    ...
    User
        .findById(user_id)
        .then(theUser => res.render('pages/user/edit-profile-page', {theUser: req.session.currentUser }))
...
router.get('/cart', isLoggedIn, checkRoles('CLIENT'),(req, res, next) => {
  ...
  const productsPromise = Product.find()
    ...
    Promise
        ...
        .then(results => res.render('pages/cart/cart',{allStores: results[0], selectedProducts: results[1]}))
        .catch(err => next(new Error(err)))
})
...
router.post('/cart', isLoggedIn, checkRoles('CLIENT'), (req, res, next) => {
    ...
    Product
        .findByIdAndUpdate(req.query.id, { name, description, type, image, price, stock })
